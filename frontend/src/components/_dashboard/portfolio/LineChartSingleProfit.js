...
export default function LineChartSingle() {
  ...
  useEffect(() => {
    axios.get(`http://127.0.0.1:8000/portfolio/time_series?portfolio_id=${params.portfolio_id}`, {
      headers: {
      ...
      if (res.data.length > 0) {
        for (let i = 0; i < res.data.length; i++) {
          ...
          let tmpList = [];
          let tmpDictData = {};
          tmpDictData["name"] = 'Portfolio Profit';
